  ...
  describe('eth_sendRawTransaction', async function () {
    ...
    describe('WRONG_NONCE Error Handling', function () {
      ...
      withOverriddenEnvsInMochaTest({ USE_ASYNC_TX_PROCESSING: false }, () => {
        ...
        it('should use nonce floor when mirror nonce is stale (floor higher than mirror)', async function () {
          // Transaction nonce = 3, mirror nonce = 1 (stale), contract results nonce = 3 (floor = 4)
          // Without nonce floor: 3 > 1 => NONCE_TOO_HIGH (wrong — user's nonce is actually low)
          // With nonce floor: max(1, 4) = 4, and 3 < 4 => NONCE_TOO_LOW (correct)
          const txWithNonce = {
            ...transaction,
            nonce: 3,
          };
          ...
          // Mock contract results endpoint — nonce=3 means floor = 4
          restMock
            .onGet(/contracts\/results\?from=/)
            .reply(200, JSON.stringify({ results: [{ nonce: 3 }], links: { next: null } }));
        ...
        it('should fall back to raw mirror nonce when nonce floor query fails', async function () {
          // Transaction nonce = 10, mirror nonce = 5, contract results endpoint fails
          // Falls back to mirror nonce only: 10 > 5 => NONCE_TOO_HIGH
          const txWithNonce = {
            ...transaction,
            nonce: 10,
          };
          ...
          restMock.resetHistory();
          restMock.onGet(accountEndpoint).reply(200, JSON.stringify({ ...ACCOUNT_RES, ethereum_nonce: 5 }));
          // Mock contract results endpoint to fail
        ...
        it('should behave normally when nonce floor matches mirror nonce', async function () {
          // Transaction nonce = 10, mirror nonce = 5, contract results nonce = 4 (floor = 5)
          // max(5, 5) = 5, and 10 > 5 => NONCE_TOO_HIGH (same result as without floor)
          const txWithNonce = {
            ...transaction,
            nonce: 10,
          };
          ...
          restMock.resetHistory();
          restMock.onGet(accountEndpoint).reply(200, JSON.stringify({ ...ACCOUNT_RES, ethereum_nonce: 5 }));
          // Mock contract results — nonce=4 means floor = 5, matching mirror nonce
          restMock
            .onGet(/contracts\/results\?from=/)
            .reply(200, JSON.stringify({ results: [{ nonce: 4 }], links: { next: null } }));
