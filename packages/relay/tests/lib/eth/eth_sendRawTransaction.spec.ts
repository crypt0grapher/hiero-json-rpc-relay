  ...
  describe('eth_sendRawTransaction', async function () {
    ...
    describe('WRONG_NONCE Error Handling', function () {
      ...
      withOverriddenEnvsInMochaTest({ USE_ASYNC_TX_PROCESSING: false }, () => {
        ...
        it('should classify against contract results floor when mirror nonce is inflated', async function () {
          // Production scenario: tx nonce 24, mirror says 24, but contract results says last successful nonce was 19
          // So the true floor is 20. WRONG_NONCE evidence causes mirror to be treated as suspect.
          const contractResultsByFromPath = `contracts/results?from=${accountAddress}&limit=1&order=desc`;
          const txWithInflatedNonce = {
            ...transaction,
            nonce: 24,
          };
          const signed = await signTransaction(txWithInflatedNonce);

          // SDK throws WRONG_NONCE error
          const wrongNonceError = new SDKClientError(
            { status: Status.WrongNonce, message: 'WRONG_NONCE' },
            'WRONG_NONCE',
            transactionIdServicesFormat,
          ...
          // Mirror reports inflated ethereum_nonce=24 (equal to tx nonce)
          restMock.resetHistory();
          restMock.onGet(accountEndpoint).reply(200, JSON.stringify({ ...ACCOUNT_RES, ethereum_nonce: 24 }));
          // Contract results: last successful nonce was 19, so floor = 20
          restMock
            .onGet(contractResultsByFromPath)
            .reply(200, JSON.stringify({ results: [{ nonce: 19, from: accountAddress }], links: { next: null } }));

          await expect(ethImpl.sendRawTransaction(signed, requestDetails))
            .to.be.rejectedWith(JsonRpcError)
            .and.eventually.satisfy(
              (error: JsonRpcError) =>
                expect(error.code).to.equal(predefined.NONCE_TOO_HIGH(24, 20).code) &&
                expect(error.message).to.include('Nonce too high'),
            );
        ...
        it('should use normal max() when no inflation (mirror matches contract results)', async function () {
          // No inflation case: tx nonce 10, mirror=5, contract results floor=5 -> max(5,5)=5
          const contractResultsByFromPath = `contracts/results?from=${accountAddress}&limit=1&order=desc`;
          const txWithHighNonce = {
            ...transaction,
            nonce: 10,
          ...
          // Mirror reports ethereum_nonce=5
          restMock.resetHistory();
          restMock.onGet(accountEndpoint).reply(200, JSON.stringify({ ...ACCOUNT_RES, ethereum_nonce: 5 }));
          // Contract results: last successful nonce was 4, so floor = 5
          restMock
            .onGet(contractResultsByFromPath)
            .reply(200, JSON.stringify({ results: [{ nonce: 4, from: accountAddress }], links: { next: null } }));
